...
import PanDetails from '../../components/semi-components/pan-detail.js';
import DetailedProduct from '../../components/semi-components/Product/detailed_product.js';
import ProductVariants from '../../components/semi-components/Product/product_variants.js';
import DetailedImages from '../../components/semi-components/Product/detailed_images.js';
import DetailedImages2 from '../../components/semi-components/Product/detailed_images2.js';
import DetailedPrice from '../../components/semi-components/Product/detailed_prices.js';
import DetailedQuantities from '../../components/semi-components/Product/detailed_quantities.js';
import DetailedOptions from '../../components/semi-components/Product/detailed_options.js';
        ...
        pan[i].addEventListener("click", () =>
        {
            ...
            {   
                ...
                //div already exists, remove it, and create another
                {
                    ...
                    rot.render( <DetailedProduct key={`${_data.title}_${i}`} Product_Title = {_data.title} Product_Category={_data.category} Product_Code={_data.product_code}
                        Product_Type={_data.product_type} Product_Vendor={_data.vendor} Product_ID={_data.id} Product_Activity={_data.active}
                        Product_Options={_data.options.map((el, i) => <DetailedOptions key={`${el.title}_${i}`} Option_Value={el.value} Option_Name = {el.position} />)}
                    />) 
                    ...
                    setTimeout(() =>
                    {
                        ...
                        for(let i = 0; i < _div.length; i++)
                        {
                            ...
                            if(i == 0) { _root.render( _data.product_images.map((el, i) => <DetailedImages key={`${el.title}_${i}`} Image1 = {el.src}/> )) }
                            else { _root.render( _data.product_images.map((el, i) => <DetailedImages2 key={`${el.title}_${i}`} Image1 = {el.src}/> )) }
                        }
                        let new_div = details.querySelector(".variants"); 
                        let rt = createRoot(new_div);
                        rt.render( _data.variants.map((el, i) => <ProductVariants key={`${el.title}_${i}`} Variant_Title = {el.id}
                        ...
                        Price={el.variant_price_tiers.map((el, i) => <DetailedPrice key={`${el.title}_${i}`} Price_Name={el.name} Price_Value={el.value}  />)}
                        Quantities={el.variant_quantities.map((el, i) => <DetailedQuantities key={`${el.title}_${i}`} quantity_value = {el.value} 
                ...
                //create new div
                {
                    ...
                    setTimeout(() =>
                    {
                        ...
                        for(let i = 0; i < _div.length; i++)
                        {
                            ...
                            if(i == 0)
                            {
                                _root.render( _data.product_images.map((el, i) => <DetailedImages key={`${el.title}_${i}`} Image1 = {el.src}/> ))
                            }
                            else 
                            {
                                _root.render( _data.product_images.map((el, i) => <DetailedImages2 key={`${el.title}_${i}`} Image1 = {el.src}/> ))
                            }
                        }
                        let new_div = details.querySelector(".variants"); 
                        let rt = createRoot(new_div);
    ...
    //If they exist remove them, and create new based on the new index value
    {
        ...
        nextPage.addEventListener("click", () =>
        {
            ...
            .done(function( _data) 
            {
                ...
                flushSync(() => 
                { 
                    root.render(_data.map((el, i) => <PanDetails key={`${el.title}_${i}`} Product_Title={el.title} Product_ID={el.id} Product_Activity={el.active}
