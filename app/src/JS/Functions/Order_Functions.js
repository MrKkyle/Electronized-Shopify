...
import $ from 'jquery';
import OrderDetails from '../../components/semi-components/order-details.js';
import DetailedOrder from '../../components/semi-components/Order/detailed_order.js';
import DetailedTableView from '../../components/semi-components/Order/detailed_table_view.js';
import DetailedTable from '../../components/semi-components/Order/detailed_table.js';
import DetailedAddress from '../../components/semi-components/Order/detailed_address.js';
        ...
        pan[i].addEventListener("click", () =>
        {
            ...
            {   
                ...
                //div already exists, remove it, and create another
                {
                    ...
                    rot.render(<DetailedOrder key={`${_data.title}_${i}`}
                    ...
                    setTimeout(() =>
                    {
                        ...
                        {
                            rt.render(<DetailedTableView key={`${_data.title}_${i}`} Sub_Total={_data.order_total} Tax_Percent={((_data.tax_total)/(_data.order_total))*100}
                            Tax_Amount={_data.tax_total} Shipping_Type={''} Shipping_Amount={''} 
                            Total={parseFloat(_data.order_total) + parseFloat(_data.tax_total)}
                            />)
                            _rt.render(_data.line_items.map((el, i) => <DetailedTable  key={`${_data.title}_${i}`} Order_SKU={el.sku} 
                            Quantity={el.qty} Barcode={el.barcode} Order_Price={el.price}
                            />))

                            let address = document.getElementById("address");
                            let rt_ = createRoot(address);
                            rt_.render(_data.customer.addresses.map((el, i) => <DetailedAddress key={`${_data.title}_${i}`}
    ...
    //If they exist remove them, and create new based on the new index value
    {
        ...
        nextPage.addEventListener("click", () =>
        {
            ...
            .done(function( _data) 
            {
                ...
                flushSync(() => 
                { 
                    root.render(_data.map((el, i) => <OrderDetails key={`${el.title}_${i}`} /> ))
                });
            })
            .fail( function(xhr) { alert(xhr.responseText); });
